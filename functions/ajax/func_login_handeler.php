    ...
    if(isset($_POST['type']) && $_POST['type'] != ''){
        if($_POST['type'] == "login"){
            if($_POST['username'] != "" && $_POST['password'] != ""){
                ...
                $selectUserCount = $db_conn->prepare("SELECT COUNT('id') AS rowCount FROM users WHERE username='".$_POST['username']."' AND password='".$password."' AND status='1'");
            ...
            if($_POST['name'] != '' && $_POST['username'] != "" && $_POST['password'] != ""){
                $selectUsername = $db_conn->prepare("SELECT COUNT(`username`) FROM users WHERE username='".$_POST['username']."'");
